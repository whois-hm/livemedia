...
{
	void close(mediacontainer &con)
	{
		for(auto  & it : con._packets)
		{
			if(it.size() > 0)
			{
				it.clear();
...
public:
	...
	_eof(false)
	{
		DECLARE_THROW(avformat_open_input(&_context,file, NULL, NULL) < 0,
				"can't open input");
		DECLARE_THROW(avformat_find_stream_info(_context, NULL) < 0,
				"can't find stream from input");
	}
	...
	mediacontainer(const mediacontainer &rhs) : _context(nullptr),
			_packets(AVMEDIA_TYPE_NB),
			_eof(false)
	{
		/*new file create*/
		DECLARE_THROW(avformat_open_input(&_context,rhs._context->filename, NULL, NULL) < 0,
				"can't open input");
		DECLARE_THROW(avformat_find_stream_info(_context, NULL) < 0,
				"can't find stream from input");
	}
	mediacontainer(mediacontainer &&rhs) : _context(rhs._context),
	...
	mediacontainer &operator = (const mediacontainer &rhs)
	{
		close(*this);
		DECLARE_THROW(avformat_open_input(&_context,rhs._context->filename, NULL, NULL) < 0,
				"can't open input");
	...
	mediacontainer &operator = (mediacontainer &&rhs)
	{
			...
			else
			{
				...
				for(unsigned i = 0; i < _context->nb_streams; i++)
				{
					if(_context->streams[i]->index == packet_ref.raw()->stream_index)
					{
						...
						_packets.at(_context->streams[i]->codec->codec_type).push_back(packet_ref);
						break;
					}
				}
			}
	...
	{
		return src ? read_packet(src->codec->codec_type, pkt) :
				false;
	}
	...
	void seek(enum AVMediaType base,/*seek by base*/
			double incr,/*increase or decrease*/
			double pts/*current pts from base*/
			)
	{
		int64_t seekpts = 0;
		int seekflags = 0;
		...
		if(targetstream)
		{
			...
			seekpts = av_rescale_q(seekpts,
					AV_TIME_BASE_Q,
					timebase(*targetstream));
   ...
   duration_div duration(bool &has)
	...
	void dump_meta(std::string &t_string)
	/*put meta info to t_string*/
	{
		bool b = false;
...
private:
	void put_metadata(const char *key,
			unsigned val,
			std::string &t_string)
	{
		char buffer[1024] = {0,};
	...
	void put_metadata(const char *linevalue,
			std::string t_string)
	{
